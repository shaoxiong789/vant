<template>
  <div class="time-picker">
    <picker ref="picker" :title="title" :columns="columns" :item-height="itemHeight" :show-toolbar="showToolbar" :visible-item-count="visibleItemCount" @change="onChange" @confirm="onConfirm" @cancel="$emit('cancel')" />
  </div>
</template>
...
export default create({
  ...
  computed: {
    ...
    columns() {
      const results = this.ranges.map(({ type, range, step }) => {
        let n = 0;
        if (type === "minute" && step !== 1) {
          n = Math.ceil(range[1] / step); // 向上取整
        } else {
          n = range[1] - range[0] + 1;
        }
        const values = this.times(n, index => {
          if (type === "minute") {
            let value = range[0] + index * step;
            if (value <= range[1]) {
